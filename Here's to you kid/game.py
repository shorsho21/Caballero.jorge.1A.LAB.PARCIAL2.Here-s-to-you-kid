...
class Game:
    def __init__(self) -> None:
        #BANDERA DE ESCRITUA DE ARCHIVO SCORE
        self.flag_score_archivo = True
    ...
    def show_game_over_screen(self):
        ...
        #VARIABLES PARA GUARDAR EL TIMER Y LUEGO ESCRIBIR EN EL ARCHIVO
        minutes = str(self.timer.time_minutes)
        seconds = str(self.timer.time_seconds)
        if self.flag_score_archivo:
            with open(r"score.txt", "w") as archivo:
                archivo.write(f"NAME, SCORE, TIME_SURVIVED\n")
                archivo.write(f"KID,{str(self.score)},{minutes:}:{seconds:}\n")
                self.flag_score_archivo = False
        else:
            with open(r"score.txt", "a") as archivo:
                archivo.write(f"KID,{str(self.score)},{minutes}:{seconds}\n")

        self.show_score_archivo()
    ...
    def show_score_archivo(self):
        with open(r"score.txt", "r") as archivo:
        ...
        #construcion de all score y su bliteo
        texto = self.font_mini.render(f"ALL SCORES", True, (BLANCO))
        texto_rect = texto.get_rect()
        texto_rect.midtop = ((WIDTH // 2) - 120, 0)
        self.display.blit(texto, texto_rect.center)


        if len(lista) >= 2:

            texto2 = self.font_mini.render(f"name:{lista[1]['name']}1 score:{lista[1]['score']} time:{lista[1]['time']}", True, (BLANCO))
            texto2_rect = texto2.get_rect()
            texto2_rect = ((WIDTH // 2) - 210, 60)
            self.display.blit(texto2, texto2_rect)
            

        if len(lista) >= 3:
            texto3 = self.font_mini.render(f"name:{lista[2]['name']}2 score:{lista[2]['score']} time:{lista[2]['time']}", True, (BLANCO))
            texto3_rect = texto3.get_rect()
            texto3_rect = ((WIDTH // 2) - 210, 120)
            self.display.blit(texto3, texto3_rect)

        if len(lista) >= 4:
            texto4 = self.font_mini.render(f"name:{lista[3]['name']}3 score:{lista[3]['score']} time:{lista[3]['time']}", True, (BLANCO))
            texto4_rect = texto3.get_rect()
            texto4_rect = ((WIDTH // 2) - 210, 180)
            self.display.blit(texto4, texto4_rect)

        if len(lista) >= 5:
            texto5 = self.font_mini.render(f"name:{lista[4]['name']}4 score:{lista[4]['score']} time:{lista[4]['time']}", True, (BLANCO))
            texto5_rect = texto3.get_rect()
            texto5_rect = ((WIDTH // 2) - 210, 180 + 60)
            self.display.blit(texto5, texto5_rect)

        if len(lista) >= 6:
            texto6 = self.font_mini.render(f"{lista[5]['name']}5 {lista[5]['score']} {lista[5]['time']}", True, (BLANCO))
            texto6_rect = texto3.get_rect()
            texto6_rect = ((WIDTH // 2) - 210, 180 + 60 + 60)
            self.display.blit(texto6, texto6_rect)





        #BLITEO DE FONDO Y ALL SCORE
        #self.display.blit(self.background_game_over, ORIGIN)
